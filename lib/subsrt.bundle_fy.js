...
todo:
1. multi-line supprts
2. proper ssa handling (parsing and mapping)

fixed:
- https://github.com/papnkukn/subsrt/issues/9
- https://github.com/papnkukn/subsrt/issues/13
******************************************************************************************/
...
function parse(content, options) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if(!match && i == 0) {  //weird bug. https://github.com/papnkukn/subsrt/issues/9
      regex = /(\d+)\r?\n(\d{1,2}:\d{1,2}:\d{1,2}([.,]\d{1,3})?)\s*\-\-\>\s*(\d{1,2}:\d{1,2}:\d{1,2}([.,]\d{1,3})?)\r?\n([\s\S]*)(\r?\n)*$/gi;
